...
Сначала подаем тактирование на модуль таймера. Мы тактируемся от *HSE*, _8 МГц_. А чтобы таймер заработал,
подключим его к системе тактирования, т.е. к шине *APB1*.

    RCC::APB1ENR::TIM2EN::Enable::Set();

Далее устанавливаем источник генерации прерываний по событию переполнений по событию переполнение
с помощью бита *URS* в регистре *CR1*

    TIM2::CR1::URS::OverflowEvent::Set();

Уставливаем значение, до которого счётчик будет считать в регистре перезагрузке
